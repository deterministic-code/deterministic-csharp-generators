...
public class MysqlCrudRepository : ICrudRepository
{
    ...
    {
        var sql = SqlBuilder.BuildSelectById(Dialect.Mysql, TableName);
    ...
    {
        var sql = SqlBuilder.BuildSelectAll(Dialect.Mysql, TableName);
    ...
    public Task<IReadOnlyList<RowMap>> FindByAsync(
        ...
        CancellationToken cancellationToken = default)
    {
        var sql = SqlBuilder.BuildSelectByColumn(Dialect.Mysql, TableName, column);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        _ = SqlBuilder.BuildInsert(Dialect.Mysql, TableName, data.Keys.ToList());
        throw new UnimplementedException("MysqlCrudRepository.AddAsync live driver is not wired");
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (data.Count > 0)
        {
            _ = SqlBuilder.BuildUpdate(Dialect.Mysql, TableName, data.Keys.ToList());
        }
        _ = id;
        throw new UnimplementedException("MysqlCrudRepository.UpdateAsync live driver is not wired");
    }

    public Task<bool> DeleteAsync(long id, CancellationToken cancellationToken = default)
    {
        _ = SqlBuilder.BuildDelete(Dialect.Mysql, TableName);
        _ = id;
        throw new UnimplementedException("MysqlCrudRepository.DeleteAsync live driver is not wired");
    }
}

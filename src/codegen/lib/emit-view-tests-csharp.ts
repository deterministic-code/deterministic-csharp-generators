...
import { emitCsharpTestClass } from "./emit-csharp-tests-shared.ts";
...
export type CsharpEmitOptions = Flatten<
  NamesForOptions & {
...
function csharpNames(opts: CsharpEmitOptions): CodegenNames {
  return namesFor({ ...opts, language: "csharp" });
}

function typeName(name: string, opts: CsharpEmitOptions): string {
  return csharpNames(opts).className(name);
}

function viewValidator(name: string, opts: CsharpEmitOptions): string {
  return `${csharpNames(opts).className(name)}Validator`;
}

function testClassName(name: string, opts: CsharpEmitOptions): string {
  return `${toCase(name, opts.fileFormat as CaseFormat)}Tests`; // lint-emitter-casing-allow: toCase
}

function propName(fieldName: string, opts: CsharpEmitOptions): string {
  return toCase(fieldName, opts.fieldFormat as CaseFormat); // lint-emitter-casing-allow: toCase
}

function dsQualified(base: string, opts: CsharpEmitOptions): string {
...
function renderUnionFacts(view: UnionView, opts: CsharpEmitOptions): string[] {
  const vcls = viewValidator(view.name, opts);
...
export function emitForView(
  view: View,
  datasource: Datasource | undefined,
  options: Partial<CsharpEmitOptions> = DEFAULT_EMIT_OPTIONS,
): EmittedFile {
  const opts: CsharpEmitOptions = { ...DEFAULT_EMIT_OPTIONS, ...options };
...
export function emitFromSchema(
  ...
  return (normalizeAll(viewTypes ?? { types: [] }) as View[]).map((v) =>
    emitForView(v, datasource, opts),
  );
}

const baseCreateEmitter = viewEmitter((view, ctx) =>
  emitForView(view, ctx.opts.datasourceTypes, ctx.opts),
);

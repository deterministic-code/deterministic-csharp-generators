...
function className(name: string, opts: CsharpEmitOptions): string {
  return csharpNames(opts).className(name);
}

function validatorName(name: string, opts: CsharpEmitOptions): string {
...
function propName(name: string, opts: CsharpEmitOptions): string {
...
/** The C# runtime expression for a {@link RuntimeValue} — baked into emitted xUnit fixtures so they use real Guid/DateTime values, not hardcoded placeholders; size-bounded strings stay within maxLength. */

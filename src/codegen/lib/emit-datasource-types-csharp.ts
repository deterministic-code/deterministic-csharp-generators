...
type CsCtx = EmitCtx<CsEmitOptions, CsharpImports>;

export const DEFAULT_EMIT_OPTIONS: CsEmitOptions = {
...
function emitField(field: DatasourceField, ctx: CsCtx): string {
...
function buildBaseSpec(opts: CsEmitOptions): {
  baseName: string;
  typeArgs: string[];
} {
...
function bodyLineFor(field: DatasourceField, ctx: CsCtx): string {
  ...
  return emitField(field, ctx);
}

function renderTable(table: NormalizedTable, ctx: CsCtx): EmittedFile {

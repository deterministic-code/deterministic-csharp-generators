...
import { DEV_PORTS, buildAppModel, deriveCSharpProjectName, } from "@deterministic-code/generator-sdk/create-backend-app-model";
import { makeChunkLoader, renderTemplate, } from "@deterministic-code/generator-sdk/codegen/lib/chunk-loader";
...
import { COMPOSE_FILENAME, renderCSharpSingleComposeService, renderCSharpComposeService, } from "@deterministic-code/generator-sdk/codegen/lib/compose-services";
import { isMultiLanguage } from "@deterministic-code/generator-sdk/codegen/lib/declared-languages";
import { PATCH, skeletonEntriesFromFiles, } from "@deterministic-code/generator-sdk/codegen/lib/generate-result";
...
const templatesDir = resolve(PACK_TEMPLATES_DIR, "create-backend-app", "csharp", "App");
const CSHARP_DIALECT_PACKAGES = {
...
const CSHARP_PACKAGES_CHUNKS = (await renderDialectMap("csharp", "packages", CSHARP_PACKAGES_TOKENS_BY_DIALECT));
export function csharpDialectPackagesContent(dialects) {
    const seen = new Set();
    const lines = [];
...
export async function renderCSharpAppCsproj(model) {
...
export async function renderCSharpProgram(model) {
    const templatePath = resolve(templatesDir, "Program.cs.tmpl");
    return await renderTemplate(templatePath, {
        PROJECT_NAME: deriveCSharpProjectName(model),
    });
}
export async function renderCSharpDockerfile(model, { multiLanguage } = {}) {
...
export async function renderCSharpAppsettings(env) {
...
export function csharpDialectUsingsContent(dialects, opts = {}) {
    ...
    if (dialects.includes("mysql"))
        lines.push(CSHARP_DIALECT_USINGS_LINES.mysql);
...
export function csharpMigrateSetupDdlConstsContent(dialects) {
...
const CSHARP_SWITCH_ARMS_CHUNKS = (await renderDialectMap("csharp", "switch_arms", CSHARP_SWITCH_ARMS_TOKENS_BY_DIALECT));
export function csharpMigrateSetupSwitchArmsContent(dialects) {
    return filterChunks(CSHARP_SWITCH_ARMS_CHUNKS, dialects, "\n");
}
const CSHARP_PROVIDER_METHOD_POSTGRES = await loadChunk("csharp", "provider_connection_string_postgres");
const CSHARP_PROVIDER_METHOD_MYSQL = await loadChunk("csharp", "provider_connection_string_mysql");
const CSHARP_PROVIDER_URL_HELPERS = await loadChunk("csharp", "provider_connection_string_url_helpers");
export function csharpProviderConnectionStringMethodsContent(dialects) {
    ...
    const out = [];
    if (wantsPostgres)
        out.push(CSHARP_PROVIDER_METHOD_POSTGRES);
    if (wantsMysql)
        out.push(CSHARP_PROVIDER_METHOD_MYSQL);
    out.push(CSHARP_PROVIDER_URL_HELPERS);
    return out.join("\n\n");
}
const CSHARP_SQLITE_PRECHECK_BLOCK = await loadChunk("csharp", "sqlite_precheck");
export function csharpMigrateSqlitePrecheckContent(dialects) {
...
const CSHARP_UP_DISPATCH_CHUNKS = (await renderDialectMap("csharp", "up_dispatch_arms", CSHARP_UP_DISPATCH_TOKENS_BY_DIALECT));
export function csharpMigrateUpDispatchArmsContent(dialects) {
...
const CSHARP_DOWN_DISPATCH_CHUNKS = (await renderDialectMap("csharp", "down_dispatch_arms", CSHARP_DOWN_DISPATCH_TOKENS_BY_DIALECT));
export function csharpMigrateDownDispatchArmsContent(dialects) {
...
export function csharpMigrateUpRunnerMethodsContent(dialects) {
...
export function csharpMigrateDownRollbackMethodsContent(dialects) {
    return filterChunks(CSHARP_DOWN_ROLLBACK_CHUNKS, dialects, "\n\n");
}
async function loadCsharpModel(inputDir, applicationName, settings) {
    return buildAppModel({
        ...(await loadBackendAppInputs(inputDir, settings)),
        applicationName,
    });
}
async function buildCsharpFiles(model, multiLanguage) {
    return [
        {
...
export async function generateBackendApp(args) {
    ...
    if (!(await pathExists(inputDir))) {
        throw new Error(`create-backend-app (csharp): input directory does not exist: ${inputDir}`);
    }
    const settings = await readSettingsWithDefault(inputDir);
    const model = await loadCsharpModel(inputDir, settings.applicationName, settings);
    ...
    const isSkeletonTarget = (path) => path.endsWith(".csproj") ||
        path === "Dockerfile" ||
        path === join("scripts", "entrypoint.sh");
    const entries = skeletonEntriesFromFiles(await buildCsharpFiles(model, multiLanguage), isSkeletonTarget);
    entries.push({
        kind: PATCH,
        ...
        content: multiLanguage
            ...
            { dockerfilePath: "./csharp/Dockerfile" })

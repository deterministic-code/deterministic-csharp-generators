...
function propName(fieldName, opts) {
    return toCase(fieldName, opts.fieldFormat); // lint-generator-casing-allow: toCase
}
...
export function generateForView(view, datasource, options = DEFAULT_GENERATE_OPTIONS) {
...
export function generateFromSchema({ viewTypes, datasource }, options = DEFAULT_GENERATE_OPTIONS) {
    const opts = { ...DEFAULT_GENERATE_OPTIONS, ...options };
    return normalizeAll(viewTypes ?? { types: [] }).map((v) => generateForView(v, datasource, opts));
}
const baseCreateGenerator = viewGenerator((view, ctx) => generateForView(view, ctx.opts.datasourceTypes, ctx.opts));

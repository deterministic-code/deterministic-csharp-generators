...
import { emitCsharpTestClass } from "./emit-csharp-tests-shared.js";
...
function viewValidator(name, opts) {
    return `${csharpNames(opts).className(name)}Validator`;
}
function testClassName(name, opts) {
    return `${toCase(name, opts.fileFormat)}Tests`; // lint-emitter-casing-allow: toCase
}
function propName(fieldName, opts) {
    return toCase(fieldName, opts.fieldFormat); // lint-emitter-casing-allow: toCase
}
function dsQualified(base, opts) {
    return `${opts.datasourceNamespace}.${csharpNames(opts).className(base)}`;
}
function csharpRefTypeInfo(base, isArray) {
    return isArray
        ? {
            type: `List<${base}>`,
            isValue: false,
...
function viewFieldCsInfo(field, opts) {
    const { parsed } = field;
    if (parsed.kind === "primitive") {
        const base = parsed.base === "string"
            ? "string"
            : parsed.base === "number"
                ? "long"
                : "bool";
        ...
        return {
            type: base,
            isValue,
            nextLiteral: parsed.base === "string"
                ? '"next"'
                : parsed.base === "number"
                    ? "42L"
                    : "false",
    ...
    return csharpRefTypeInfo(csharpNames(opts).className(parsed.base), parsed.isArray);
}
function datasourceInheritedFields(inherits, datasource) {
    const entry = (datasource?.types ?? []).find((e) => Object.keys(e)[0] === inherits);
    if (!entry)
        return [];
    ...
    const standard = STANDARD_COLUMNS.map((c) => ({
        name: c.name,
        isNullable: false,
        isValue: VALUE_TYPES.has(c.type),
        nextLiteral: c.type === "number"
            ? "42L"
            : c.type === "datetime"
                ? "System.DateTime.UtcNow.AddDays(1)"
                : '"next"',
...
function shapedAccessorFields(view, datasource, opts) {
    const omit = new Set((view.enrichments ?? []).map((e) => e.fkColumn));
    const inherited = view.inherits
        ? datasourceInheritedFields(view.inherits, datasource).filter((f) => !omit.has(f.name))
        : [];
...
function renderShapedValidatorFacts(view, opts) {
...
function renderShapedAccessorFacts(view, datasource, opts) {
    const cls = `${opts.typesNamespace}.${typeName(view.name, opts)}`;
...
function renderUnionFacts(view, opts) {
    const vcls = viewValidator(view.name, opts);
    const facts = view.members.map((member) => {
        const memberCls = `${opts.typesNamespace}.${typeName(member, opts)}`;
        const methodName = `DispatchesTo${toCase(member, "Pascal")}Member`; // lint-emitter-casing-allow: toCase
        return [
            `    [Fact]`,
...
export function emitForView(view, datasource, options = DEFAULT_EMIT_OPTIONS) {
    ...
    return emitCsharpTestClass(testClass, header, facts);
}
export function emitFromSchema({ viewTypes, datasource }, options = DEFAULT_EMIT_OPTIONS) {
    const opts = { ...DEFAULT_EMIT_OPTIONS, ...options };
    return normalizeAll(viewTypes ?? { types: [] }).map((v) => emitForView(v, datasource, opts));
}
const baseCreateEmitter = viewEmitter((view, ctx) => emitForView(view, ctx.opts.datasourceTypes, ctx.opts));
export const createEmitter = () => {
    const base = baseCreateEmitter(CsharpImports);
    return {
